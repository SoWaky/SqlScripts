-- SELECT * FROM vw_StatsByQuarter

------------------------------------------------------------
-- The vw_StatsByQuarter View includes all fields from the CompanyStatsByMonth table, except for the Company fields
--	It sums aggregate fields and adds calculated fields at the end
--	They roll up to Year + Quarter
------------------------------------------------------------

USE MSP_Dashboard
GO

IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vw_StatsByQuarter]'))
	DROP VIEW dbo.vw_StatsByQuarter
GO

CREATE VIEW dbo.vw_StatsByQuarter
AS 
SELECT StatsYear
