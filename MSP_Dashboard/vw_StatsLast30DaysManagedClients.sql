-- SELECT * FROM vw_StatsLast30DaysManagedClients
...
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vw_StatsLast30DaysManagedClients]'))
	DROP VIEW dbo.vw_StatsLast30DaysManagedClients
GO

CREATE VIEW dbo.vw_StatsLast30DaysManagedClients
AS 
SELECT SUM(Num_Seats) AS Num_Seats
		...
		, ((SUM(MRR_Amount) + SUM(ORR_Amount)) / COUNT(*)) AS Avg_MRR_Amount
		, COUNT(*) AS Num_Companies
	FROM MSP_Dashboard.dbo.CompanyStatsLast30Days ms WITH (NOLOCK)
	WHERE Company_Type = '10 Client - Managed Services'
GO