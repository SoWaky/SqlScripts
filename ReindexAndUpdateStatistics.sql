...
WHILE @@fetch_status = 0
 ...
 INSERT INTO #tmpFragmentedIndexes (
  ...
  SELECT
     ...
     ''ALTER INDEX [''+idx.Name+''] ON [''+DB_NAME()+''].[''+SCHEMA_NAME (tbl.schema_id)+''].[''+tbl.name+''] REBUILD WITH (FILLFACTOR = '+@indexFillFactor+', SORT_IN_TEMPDB = '+@sortInTempdb+', STATISTICS_NORECOMPUTE = OFF, MAXDOP=1);''
...
ELSE
BEGIN
 RAISERROR ('Error: You need to be SysAdmin and use SQL Server 2005 or later in order to use this script.',16,1);  
END
--End of Script